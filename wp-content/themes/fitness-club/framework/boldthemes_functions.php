	...
	function boldthemes_header_headline( $arg = array() ) {
		...
		if ( ( ! $hide_headline && ! is_404() ) ) {
			...
			if ( $title != '' || $supertitle != '' || $subtitle != '' ) {
				...
				echo '<section class="boldSection bottomSemiSpaced btPageHeadline gutter ' . $extra_class . '" style="background-image:url(' . $feat_image . ')" data-parallax="' . esc_attr( $parallax ) . '" data-parallax-offset="0"><div class="port">';
						...
						<?php
							...
							if ( $rating > 0 && get_option( 'woocommerce_enable_review_rating' ) == 'yes' ) { ?>
								<div itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating" class="star-rating" title="<?php echo sprintf( esc_html__( 'Rated %d out of 5', 'woocommerce' ), $rating ) ?>">
									<span style="width:<?php echo wp_kses_post( ( $rating / 5 ) * 100 ); ?>%"><strong itemprop="ratingValue"><?php echo wp_kses_post( $rating ); ?></strong> <?php esc_html_e( 'out of 5', 'woocommerce' ); ?></span>
								</div>
							<?php }
						?>
					</div>
	...
	function boldthemes_get_post_categories( $arg = array() ) {
		...
		if ( $categories ) {
			...
			foreach ( $categories as $cat ) {
				if ( ! $no_link ) $categories_html .= '<a href="' . esc_url_raw( get_category_link( $cat->term_id ) ) . '" class="btArticleCategory">';
